...
def handle_upload(
    ...
    for file_obj in files_to_process:
        try:
            ...
            logger.info(f"Successfully processed file: {filename} (document_id: {document_id})")
            success_count += 1
            ...
            error_msg = f"Error processing {getattr(file_obj, 'name', 'unknown')}: {str(e)}"
...
def handle_question(
    ...
    try:
        ...
        if USE_FUNCTION_CALLING:
            ...
            logger.info("Using Function Calling mode - AI will call tools to fetch data")
...
def handle_clear_session(session_id: str) -> int:
    ...
    try:
        ...
        logger.info(f"Successfully cleared {deleted_count} document(s) from session: {session_id}")
