...
from backend.config import (
    ...
    EMBEDDING_COLUMN,
    CHUNK_CONTENT_COLUMN, # New import
)
...
def init_db() -> None:
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            logger.info(f"Creating {DOCUMENT_CHUNKS_TABLE} table")
            conn.execute(
                text(f"""
                CREATE TABLE IF NOT EXISTS {DOCUMENT_CHUNKS_TABLE} (
                    ...
                    {EMBEDDING_COLUMN} VECTOR({EMBEDDING_DIMENSION}) NOT NULL,
            ...
            conn.execute(
                ...
                ALTER TABLE {DOCUMENT_CHUNKS_TABLE} ADD COLUMN IF NOT EXISTS
                    lsh_signature TEXT;
            """)
            )
            ...
            logger.info(f"Creating IVFFlat index on {EMBEDDING_COLUMN} column in {DOCUMENT_CHUNKS_TABLE}")
            conn.execute(
                text(f"""
    ...
    try:
        with engine.connect() as conn:
            ...
            for idx, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
                ...
                query = f"""
                    INSERT INTO {DOCUMENT_CHUNKS_TABLE} (document_id, chunk_index, content, {EMBEDDING_COLUMN})
...
def search_similar_chunks(
    ...
    try:
        with engine.connect() as conn:
            ...
            query = f"""
                SELECT dc.{CHUNK_CONTENT_COLUMN}
                FROM {DOCUMENT_CHUNKS_TABLE} dc
                ...
                ORDER BY dc.{EMBEDDING_COLUMN} <-> '{embedding_str}'::vector
                LIMIT :top_k
            """
...
def get_unindexed_chunks(session_id: Optional[str] = None) -> List[Tuple[int, int, List[float]]]:
    ...
    NOTE: This version is modified to work with a single table (e.g., 'demo')
    that lacks the 'document_id' foreign key and 'lsh_indexed' flag. It assumes
    all chunks in the table are unindexed and uses the chunk ID as a placeholder
    for the document ID for the LSHIndexer logic.
    ...
    logger.info(f"Retrieving unindexed chunks from {DOCUMENT_CHUNKS_TABLE}.")

    try:
        with engine.connect() as conn:
            # Query to get all chunks from the single table
            query = f"""
                SELECT
                    id,
                    id, -- Use id as a placeholder for document_id
                    {EMBEDDING_COLUMN}
                FROM {DOCUMENT_CHUNKS_TABLE}
                ORDER BY id
            """
            # NOTE: session_id filtering is not possible without a session_id column in the table

            result = conn.execute(text(query))
            
            chunks = []
            for chunk_id, document_id_placeholder, embedding_str in result.fetchall():
                ...
                chunks.append((chunk_id, document_id_placeholder, embedding))
...
def get_chunk_content_by_ids(chunk_ids: List[int]) -> List[str]:
    ...
    logger.info(f"Retrieving content for {len(chunk_ids)} chunks from {DOCUMENT_CHUNKS_TABLE}.")
    ...
    try:
        with engine.connect() as conn:
            ...
            query = f"""
                SELECT id, {CHUNK_CONTENT_COLUMN}
                FROM {DOCUMENT_CHUNKS_TABLE}
            ...
            # The content is the second column (index 1) in the result set
