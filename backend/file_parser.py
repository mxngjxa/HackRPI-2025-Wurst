...
def validate_file(file_obj: Any) -> None:
    ...
    try:
        ...
        elif hasattr(file_obj, 'name') and file_obj.name and os.path.exists(file_obj.name):
            file_size = os.path.getsize(file_obj.name)
    except Exception as e:
        error_msg = f"Error determining file size for {os.path.basename(filename)}: {str(e)}"
...
def read_txt_file(file_obj: Any) -> str:
    ...
    try:
        ...
        raise FileValidationError(f"Permission denied reading file: {os.path.basename(file_path)}")
    except Exception as e:
        raise FileValidationError(f"Error reading file {os.path.basename(file_path)}: {str(e)}")
