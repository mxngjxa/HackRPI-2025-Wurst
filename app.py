...
log_file = os.getenv("LOG_FILE", "app.log")  # Set to empty string to disable file logging
setup_logging(
    log_level=log_level,
...
def on_upload(
    ...
    try:
        ...
        if success_count > 0 and not errors:
            ...
            logger.info(f"Upload successful: {success_count} files (session: {session_id})")
        ...
        logger.error(f"Unexpected upload error (session: {session_id}): {str(e)}", exc_info=True)
        history.append([None, error_msg])
        return (history, None)
